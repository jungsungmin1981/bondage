...
const getCachedAdvancedPosts = unstable_cache(
  () => getPublicClassPostsByLevel("advanced"),
  ["class-posts-advanced"],
  { revalidate: 60 },
);
...
export default async function ClassAdvancedPage() {
  ...
  const rows = await getCachedAdvancedPosts();
  ...
  return (
    <div className="min-h-[calc(100svh-3.5rem)] bg-background p-4 sm:p-6">
      <h1 className="text-xl font-semibold sm:text-2xl">클래스 · 고급</h1>
      <p className="mt-2 text-sm text-muted-foreground">
        고급 클래스 콘텐츠를 카드에서 선택해 보세요.
      </p>
      {cards.length === 0 ? (
        <p className="mt-6 text-sm text-muted-foreground">
          등록된 고급 클래스가 없습니다. (관리자에서 공개로 등록해 주세요.)
        </p>
      ) : (
