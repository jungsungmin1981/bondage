...
import {
  createSharedBoardNoticePostAction,
  updateSharedBoardNoticePostAction,
  deleteSharedBoardNoticePostFormAction,
...
function SubmitButton({ isEdit }: { isEdit: boolean }) {
  ...
  return (
    <Button
      ...
      disabled={pending}
    >
      {pending ? (isEdit ? "저장 중…" : "등록 중…") : isEdit ? "저장" : "공지 등록"}
    </Button>
  );
}
...
export function AdminNoticeRiggerPostForm({
  postId,
  initialValues,
}: NoticePostFormProps = {}) {
  const isEdit = !!postId;
  ...
  const boundUpdateAction = useCallback(
    (prev: unknown, formData: FormData) =>
      postId
        ? updateSharedBoardNoticePostAction(postId, prev, formData)
        ...
        <section className="grid grid-cols-1 gap-4 rounded-lg border border-border bg-muted/20 p-4 sm:p-5 md:grid-cols-[minmax(0,1fr)_minmax(0,1.2fr)]">
          <div className="space-y-3">
            ...
            <label
              ...
              className="relative flex aspect-square w-full max-w-sm cursor-pointer overflow-hidden rounded-lg border border-border bg-muted/30 min-h-[200px] md:max-w-none"
            >
              {coverPreview ? (
                <img
                  src={coverPreview}
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className="grid grid-cols-[auto_1fr] gap-x-4 gap-y-3 items-center">
              <Label
                htmlFor="admin-notice-rigger-visibility"
                className="min-w-[7rem]"
              >
                공개유무 <span className="text-red-600">*</span>
              </Label>
              <div
                id="admin-notice-rigger-visibility"
              ...
              <Label
                htmlFor="admin-notice-rigger-scheduled"
                className="min-w-[7rem]"
              >
                예약 공개 시각
              </Label>
              <div className="min-w-0">
                <ScheduledDateTimePicker
                  id="admin-notice-rigger-scheduled"
              ...
              <Label
                htmlFor="admin-notice-rigger-title"
                className="min-w-[7rem]"
              >
            ...
            <div className="space-y-2 flex-1 min-h-0">
              <Label htmlFor="admin-notice-rigger-body">내용</Label>
              <Textarea
                id="admin-notice-rigger-body"
                name="body"
                required
                maxLength={BODY_MAX}
