...
import {
  createBunnyBoardQnaPostAction,
  updateBunnyBoardQnaPostAction,
  deleteBunnyBoardQnaPostFormAction,
...
type AdminBunnyQnaPostFormProps = {
  postId?: string;
  initialValues?: {
    title: string;
    body: string;
...
export function AdminBunnyQnaPostForm({
  postId,
  initialValues: initialValuesProp,
}: AdminBunnyQnaPostFormProps = {}) {
  const isEdit = !!postId;
  ...
  const boundUpdateAction = useCallback(
    (prev: unknown, formData: FormData) =>
      postId
        ? updateBunnyBoardQnaPostAction(postId, prev, formData)
      ...
      >
        <div key={formKey} className="flex flex-col gap-4">
          <div className="flex flex-wrap items-center gap-2">
            <Label
              htmlFor="admin-bunny-qna-visibility"
              className="shrink-0"
            >
              공개 유무 <span className="text-red-600">*</span>
            </Label>
            <div
              id="admin-bunny-qna-visibility"
          ...
          <div className="flex flex-wrap items-center gap-2">
            <Label htmlFor="admin-bunny-qna-sortOrder" className="shrink-0">
              게시물 정리 순서
            </Label>
            <Input
              id="admin-bunny-qna-sortOrder"
              name="sortOrder"
              type="number"
              min={0}
              step={1}
        ...
        <div className="space-y-2">
          <Label htmlFor="admin-bunny-qna-title">제목</Label>
        <Input
          id="admin-bunny-qna-title"
          name="title"
          required
          maxLength={TITLE_MAX}
          ...
          {isEdit && (
            <>
              ...
              <Button asChild variant="outline" className="min-h-[44px]">
                <Link href="/admin/notice/bunny-qna">취소</Link>
              </Button>
            </>
          )}
        </div>
