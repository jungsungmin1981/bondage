...
import { getBunnyBoardBySlug, getBunnyBoardPosts, getBunnyBoardPostCount } from "@workspace/db";
...
export default async function AdminNoticeBunnyPage() {
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <p className="text-sm text-muted-foreground">{postCount}개의 공지</p>
        <Button asChild className="min-h-[44px] shrink-0">
          <Link href="/admin/notice/bunny/new">
          ...
          {posts.map((post) => (
            <li key={post.id}>
              <div className="flex flex-wrap items-center gap-2 rounded-xl border border-border bg-card p-4 transition-colors hover:border-muted-foreground/30 hover:bg-muted/30 sm:flex-nowrap">
                <Link
                  href={`/admin/notice/bunny/${post.id}/edit`}
                ...
                >
                  ...
                  <span className="min-w-0 flex-1">
                    ...
                    <p className="mt-1 text-xs text-muted-foreground">
                      수정자: {post.updatedByNickname ?? "-"} ·{" "}
                      {formatDate(post.updatedAt)}
                    </p>
                  </span>
                </Link>
                <Link
                  href={`/bunnies/board/notice/${post.id}`}
                  target="_blank"
