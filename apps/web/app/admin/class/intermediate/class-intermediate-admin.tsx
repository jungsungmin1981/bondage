...
export function ClassIntermediateAdmin() {
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      const coverUrl = coverFile ? await uploadImageToS3(coverFile, id) : form.imageUrl;
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-lg font-semibold">클래스 · 중급</h2>
        <p className="mt-1 text-sm text-muted-foreground">
          중급 클래스 게시물을 등록·관리합니다.
        </p>
      </div>
            ...
            <div className="relative">
              <Input
                ...
                onChange={(e) =>
                  setForm((f) => ({
                    ...f,
                    title: e.target.value.slice(0, TITLE_MAX_LEN),
                  }))
                }
                placeholder="예: 중급 클래스 제목"
                className="pr-14"
              />
            ...
            <div className="space-y-2">
              <input
                ...
                onChange={async (e) => {
                  ...
                  const remain = MAX_EXTRA_IMAGES - form.extraImageUrls.length;
                  if (remain <= 0) {
                    e.target.value = "";
                    return;
                  }
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            {filteredItems.map((item) => (
              <div key={item.id} className="relative">
                ...
                >
                  <ClassCard card={item} levelLabel="중급" />
                </button>
              </div>
            ))}
          </div>
          ...
          <AlertDialogHeader className="place-items-center text-center sm:place-items-center sm:text-center">
            ...
            <AlertDialogDescription className="flex flex-col items-center justify-center gap-2 text-center">
              <span className="max-w-[22rem] text-base leading-relaxed text-foreground">
                완전히 <span className="font-semibold text-red-600">삭제</span>하면 복구할 수 없습니다.
              </span>
              <span className="text-base font-semibold text-foreground">
                <span className="font-semibold text-red-600">삭제</span> 하시겠습니까?
              </span>
            </AlertDialogDescription>
          </AlertDialogHeader>
