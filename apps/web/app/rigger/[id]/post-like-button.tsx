...
import { getPostLikers, togglePostLike } from "./post-like-actions";
...
export function PostLikeButton({
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!cancelled && result.ok) setLikers(result.likers);
    })();
    return () => {
      cancelled = true;
    };
  }, [isOwnPost, open, postId]);
  ...
  const handleClick = async (e: React.MouseEvent) => {
    ...
    setPending(true);
    try {
      const result = await togglePostLike(riggerId, postId);
      if (result.ok) {
