...
export default async function BunnyBoardPostDetailPage({
  params,
}: {
  ...
  const isNoticeNotVisible =
    ...
    (!post.isPublished ||
      (post.scheduledPublishAt && new Date(post.scheduledPublishAt) > new Date()));
    ...
    <div className="mx-auto min-h-[calc(100svh-3.5rem)] w-full max-w-2xl p-4 sm:p-6">
      ...
      {post.coverImageUrl ? (
        <div className="mb-4 overflow-hidden rounded-xl border border-border bg-card">
          <img
            ...
            alt=""
            className="w-full object-contain max-h-[50vh] bg-muted/20"
          />
        </div>
      ) : null}
      ...
      <div className="mt-4 flex flex-wrap items-center gap-2">
        <Button asChild variant="outline" className="min-h-[44px]" size="sm">
          <Link href={`/bunnies/board/${encodeURIComponent(boardSlug)}`}>
            목록으로
          </Link>
        </Button>
        {isFreeBoard && (
        ...
        {isAuthor && boardSlug === "free" && (
          <>
            <Button asChild className="min-h-[44px]" size="sm">
              <Link href={`/bunnies/board/free/${postId}/edit`}>
