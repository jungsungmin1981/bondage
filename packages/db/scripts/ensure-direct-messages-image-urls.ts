/**
 * direct_messages 테이블에 image_urls 컬럼이 없으면 추가.
 * 실행:
 *   pnpm --filter @workspace/db run db:ensure-direct-messages-image-urls
 */
...
async function main() {
  ...
  await db.execute(sql.raw(`ALTER TABLE "direct_messages" ADD COLUMN "${COLUMN}" jsonb`));
