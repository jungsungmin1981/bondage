/**
 * member_profiles 테이블에 rejection_note 컬럼이 없으면 추가.
 * "column rejection_note does not exist" 발생 시 실행:
 *   pnpm --filter @workspace/db run db:ensure-member-profiles-rejection-note
 */
...
async function main() {
  ...
  await db.execute(sql.raw(`ALTER TABLE "member_profiles" ADD COLUMN "${COLUMN}" text`));
